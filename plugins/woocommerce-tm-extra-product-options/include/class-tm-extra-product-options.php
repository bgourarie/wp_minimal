...
final class TM_Extra_Product_Options {
	...
	public final function tm_remove_wcml(){
		...
		if(TM_EPO_WPML()->is_active() && $woocommerce_wpml && $woocommerce_wpml->compatibility && $woocommerce_wpml->compatibility->extra_product_options){
	...
	public function add_plugin_actions(){
		...
		add_action( 'wp_footer', array( $this, 'tm_add_inline_style' ), 99999 );
	...
	public function tm_epo_price2($price="",$type=""){
		...
		if(in_array((string)$type, array('', 'char', 'step', 'currentstep', 'charnofirst', 'fee', 'subscriptionfee'))) {
			if (class_exists('WOOCS')){
				...
				$price=(double)$price* $currencies[$current_currency]['rate'];
			}elseif(TM_EPO_WPML()->is_active() && $woocommerce_wpml && $woocommerce_wpml->multi_currency){
				...
				//$price=$woocommerce_wpml->multi_currency->convert_price_amount($price);
	...
	public function get_price_for_cart($price=0,$cart_item=array(),$symbol=false,$currencies=NULL,$quantity_divide=0){
		global $woocommerce;
		$product 			= $cart_item['data'];
	...
	public function get_item_data_array( $other_data, $cart_item=array() ) {
		...
		if (is_array($filtered_array)){
			foreach ( $filtered_array as $section ) {
				...
				if (isset($section['other_data'] ) && is_array($section['other_data'] )){
					foreach ( $section['other_data'] as $key=>$data ) {
						...
						if (!empty($data['images']) && $this->tm_epo_show_image_replacement=="yes"){
							...
							$display_value ='<span class="cpf-img-on-cart"><img alt="" class="attachment-shop_thumbnail wp-post-image epo-option-image" src="'.
							$data['images'].'" /></span>'.$display_value. '<small>'.$format_price . $quantity_string . '</small>';
				...
				$other_data[] = array(
					'name'    			=> $section['label'],
					'value'   			=> $image_string.do_shortcode(html_entity_decode ($value)). '<small>'.(!$format_price_shown?$format_price:'') . $quantity_string . '</small>',
	...
	*/
	...
	private function print_price_fields( $product_id=0, $form_prefix="") {		
		...
		if ($this->is_quick_view() || $this->is_bto || $this->tm_epo_global_load_generated_styles_inline=="yes"){
			remove_action( 'wp_footer', array( $this, 'tm_add_inline_style' ), 99999 );
