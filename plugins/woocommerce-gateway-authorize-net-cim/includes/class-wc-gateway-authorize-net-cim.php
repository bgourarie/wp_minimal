...
class WC_Gateway_Authorize_Net_CIM extends SV_WC_Payment_Gateway_Direct {
	...
	protected function get_order( $order_id ) {
		...
		// add shipping address ID for profile transactions
		if ( $order->get_user_id() && ! empty( $order->payment->token ) ) {
	...
	protected function get_order_meta( $order_id, $key ) {
		...
		if ( 'customer_id' === $key && ! metadata_exists( 'post', $order_id, $this->get_order_meta_prefix() . $key ) &&
			...
			$token = get_post_meta( $order_id, '_wc_authorize_net_cim_customer_profile_id', true );
